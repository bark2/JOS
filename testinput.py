...
def test_testinterrupt_helper(count):
    save_pcap_on_fail()
    maybe_unlink("qemu.pcap")
    ...
    r.user_test("net_testinput",
                call_on_line("Waiting for packets",
                              lambda _: send_thread.start()),
                stop_on_line(final),make_args=[" QEMUEXTRA= -d int -D qemu.log "])
    if send_thread.isAlive():
        send_thread.join()
    ...
    match_packet_seq(got, zip(names, pcap))
    import subprocess
    task = subprocess.Popen("grep -c \"v=2b\" qemu.log", shell=True, stdout=subprocess.PIPE)
    data = task.stdout.read()
    assert int(data) != 0, "no interrupts"


@test(15, "testinterrupt")
def test_testinterrupt_5():
    test_testinterrupt_helper(5)
